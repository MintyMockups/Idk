...
function setBool(player, key, value) {
...
function botEntities(player) {
  try {
    return player.dimension.getEntities({ type: 'npvp:pvp_bot' });
  } catch {
    try { return player.dimension.getEntities({ type: 'verxhade:pvp_bot' }); } catch { return []; }
  }
}
...
async function openSettings(player) {
  ...
  try {
    ...
    msg(player, 'Could not open settings.');
  }
}
...
function spawnBot(player, difficulty = 'medium') {
  try {
    spawnPvPBot(player, difficulty);
    system.runTimeout(() => refreshBotCount(player), 2);
    msg(player, `PvP bot spawned §7(${difficulty}).`);
  } catch (error) {
    msg(player, 'Bot spawn failed. Check the bot entity/pack version.');
  }
}

function spawnDummy(player) {
  try {
    spawnPracticeDummy(player);
    msg(player, 'Practice dummy spawned.');
  } catch {
    msg(player, 'Dummy spawn failed.');
  }
}

function clearBots(player) {
  try {
    removeAllBots();
    player.setDynamicProperty(PROP.bots, 0);
    msg(player, 'All NPVP bots cleared.');
  } catch {
    msg(player, 'Could not clear bots.');
  }
}

function help(player) {
  tell(player, `§b§lNPVP COMMANDS§r\n§f!npvp open §7- open client menu\n§f!npvp help §7- show commands\n§f!npvp settings §7- configure client\n§f!npvp bots §7- show active bots\n§f!npvp spawn [easy|medium|hard] §7- spawn bot\n§f!npvp dummy §7- spawn practice dummy\n§f!npvp clear §7- remove all bots\n§f!npvp stats §7- show PvP stats\n§f!npvp hud [on|off] §7- toggle HUD\n§f!npvp toggle <hud|stats|health> [on|off]\n§f!npvp reset §7- reset NPVP settings\n§f!npvp version §7- show version`);
}

function setHud(player, value) {
  setBool(player, PROP.hud, value);
  msg(player, `HUD ${value ? 'enabled' : 'disabled'}.`);
}

function reset(player) {
  setBool(player, PROP.hud, defaults.hud);
  setBool(player, PROP.stats, defaults.stats);
  setBool(player, PROP.autoHealth, defaults.autoHealth);
  msg(player, 'NPVP settings reset to defaults.');
}
...
function runCommand(player, raw) {
  ...
  if (args.shift()?.toLowerCase() !== '!npvp') return false;
  const command = (args.shift() ?? 'help').toLowerCase();

  switch (command) {
    case 'open': return openMenu(player), true;
    case 'help': case '?': return help(player), true;
    case 'settings': case 'config': return openSettings(player), true;
    case 'bots': {
      const count = refreshBotCount(player);
      msg(player, `Active bots: §f${count}`);
      return true;
    }
    case 'spawn': return spawnBot(player, ['easy', 'medium', 'hard'].includes(args[0]?.toLowerCase()) ? args[0].toLowerCase() : 'medium'), true;
    case 'dummy': case 'target': return spawnDummy(player), true;
    case 'clear': case 'remove': return clearBots(player), true;
    case 'stats': return showStats(player), true;
    case 'hud': {
      const value = parseBool(args[0]?.toLowerCase());
      if (value === null) msg(player, `HUD is currently ${getBool(player, PROP.hud) ? 'ON' : 'OFF'}. Use §f!npvp hud on§r or §f!npvp hud off§r.`);
      else setHud(player, value);
      return true;
    }
    case 'toggle': {
      const module = args[0]?.toLowerCase();
      const value = parseBool(args[1]?.toLowerCase());
      const map = { hud: PROP.hud, stats: PROP.stats, health: PROP.autoHealth, autohealth: PROP.autoHealth };
      if (!map[module] || value === null) {
        msg(player, 'Usage: !npvp toggle <hud|stats|health> <on|off>');
        return true;
      }
      setBool(player, map[module], value);
      msg(player, `${module} ${value ? 'enabled' : 'disabled'}.`);
      return true;
    }
    case 'reset': return reset(player), true;
    case 'version': msg(player, `NPVP Client §b${VERSION}§r • command edition`), true;
    case 'info': msg(player, 'NPVP Client is a PvP practice utility with bots, targets, HUD and persistent settings.'), true;
...
world.afterEvents.playerSpawn.subscribe((event) => {
  ...
  const sessions = Number(player.getDynamicProperty('npvp:sessions') ?? 0) + 1;
  player.setDynamicProperty('npvp:sessions', sessions);
...
world.afterEvents.entityHitEntity?.subscribe((event) => {
  ...
  if (!getBool(attacker, PROP.stats)) return;
  const stats = getStats(attacker);
  stats.hits++;
  saveStats(attacker, stats);
});
...
world.afterEvents.entityDie?.subscribe((event) => {
  ...
  if (dead.typeId === 'minecraft:player') {
    const stats = getStats(dead);
    stats.deaths++;
    saveStats(dead, stats);
  }
  const killer = event.damageSource?.damagingEntity;
  if (killer?.typeId === 'minecraft:player' && dead.typeId !== 'minecraft:player') {
    const stats = getStats(killer);
    stats.kills++;
    saveStats(killer, stats);
  }
});
...
system.runInterval(() => {
  for (const player of world.getPlayers()) {
    ...
    const bots = refreshBotCount(player);
    const stats = getStats(player);
    const text = `§bNPVP §8• §fBots §b${bots} §8• §fK/D §b${stats.kills}/${stats.deaths}`;
